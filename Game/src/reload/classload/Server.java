...
public class Server {
	...
	String codePath = "F:\\workspace\\Game\\bin\\";
	String busServiceClass = "reload.classload.BusServiceImpl";
					...
					if (lastTime != f.lastModified()) {
						...
						try {
							...
							if (c instanceof URLClassLoader)
								((URLClassLoader) c).close();// 释放资源
							System.out.println("busService:" + busService + "  ,classloader:"
									+ busService.getClass().getClassLoader());
